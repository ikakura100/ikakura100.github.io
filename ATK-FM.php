<?php // been updated automatically on 2023/02/04 - Feb (Sat) 23:32:52(UNIXTIME:1675521172) from https://www.activetk.cf/ATK-FM/update/go/1.2.1.php
  /////////////////////////////////////////////////
  define('fmversion', 'v1.2.1');
  define('lastupdateinfo', '2023/02/04 - Feb (Sat) 23:32:52');
  define('publishupdateinfo', '2021/11/20 - Nov (Sat) 22:26:31');
  define('copyright',
  ...
  */
  ...
  define("phpstarttime", $phpstarttime);
  ini_set("display_startup_errors", 0);
  error_reporting(E_ALL);
  ini_set('session.gc_divisor', 1);
  ini_set('session.gc_maxlifetime', 7776000);
  if (defined('lastupdateinfo') && lastupdateinfo == "BETA!")
  {
    define('DEBUGMODE', true);
    $senddeddebugheader = 0;
  }
  else
    define('DEBUGMODE', false);
  function Debug_Write(string $message = "") {
    if (DEBUGMODE && !headers_sent())
    {
      global $senddeddebugheader;
      $senddeddebugheader++;
      header("X-ATKFM-Debug-" . str_pad($senddeddebugheader, 5, 0, STR_PAD_LEFT) . ": Time=" . (microtime(true) - phpstarttime) . " Message=" . $message);
    }
  }
  Debug_Write("[INFO] Loaded Settings");
  session_start();
  Debug_Write("[INFO] Loaded session_start()");
...
<? }
  ...
  function download($weburl, $filepath){
    ...
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Linux; ATK-FM AccessBot 1.2; PHP) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121');
    curl_exec($ch);
    curl_close($ch);
    fclose($fp);
  }
  function downloadtext($url){
    ...
    curl_setopt($curl, CURLOPT_USERAGENT, 'Mozilla/5.0 (Linux; ATK-FM AccessBot 1.2; PHP) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121');
  ...
  Debug_Write("[INFO] Loaded Functions");
  ...
  else if (isset($_POST["login"]) && isset($_POST["password"]))
  {
    if ($_POST["password"] == "" && isset($_POST["_trykey"]))
    {
      ...
      if (md5(fm_password_md5 . $_SESSION["logincode"]) == $_POST["_trykey"])
      {
        ...
        exit("<!-- ".time()." -->\n<p>LoginError: パスワードが違います <a href=''>&gt; 戻る</a></p>\n");
    }
    else
  ...
  Debug_Write("[INFO] Loaded Login event");
  if (isset($_GET["ajax-typeof"]) || isset($_GET["ajaxtypeof"]))
  {
    ...
    if ($command == "get-directory" || $command == "getdirectory")
    {
      ...
      if (!is_file(substr($hiki, 0, -1))) {
        Debug_Write("[INFO] [get-directory] Loaded session_write_close()");
        $listofs = glob("{$hiki}{*,.*}", GLOB_BRACE);
        Debug_Write("[INFO] [get-directory] Loaded glob()");
        $listof = array();
        $access_ok = false;
        ...
        {
          Debug_Write("[INFO] [get-directory] Found ({$file})");
          if (is_file($file))
          {
            $sds = pathinfo($file);
            if (isset($sds["extension"]))
              $scc = $sds["extension"];
            else $scc = "";
            if ($scc == "zip" || $scc == "7z" || $scc == "rar" || $scc == "gz" || $scc == "bz2" || $scc == "lzh")
              $listof += array($file=>'c');
            else if ($scc == "atkfm-link")
              $listof += array($file=>'e');
            else if ($scc == "atkfm-encrypt")
              $listof += array($file=>'f');
            else
              $listof += array($file=>'a');
          }
    ...
    else if ($command == "get-item" || $command == "getitem")
    {
      ...
      if (File_Exists($hiki))
      {
        if (isset($_GET["accept"]))
        {
          ...
          header('Content-Disposition: inline; filename="' . basename($hiki) . '"');
    ...
    else if ($command == "get-item-pre" || $command == "getitempre")
    {
      session_write_close();
      if (File_Exists($hiki))
      {
        echo "<pre>";
        echo htmlspecialchars(file_get_contents($hiki));
        exit("</pre>");
      }
      else exit("エラー: ファイルが見つかりません。");
    }
    else if ($command == "get-linkto" || $command == "getlinkto")
    {
      session_write_close();
      if (File_Exists($hiki))
        exit(rawurlencode(file_get_contents($hiki)));
      else
        exit();
    }
    ...
    else if ($command == "view-hex" || $command == "viewhex")
    {
      session_write_close();
      if (File_Exists($hiki))
      {
        if ($hiki2 != "")
          echo "<base href='{$hiki2}'>";
        echo "<h1>HexDump#Beta - ATK-FM</h1><pre>File: " . htmlspecialchars($hiki) . "</pre>";
        $bin = bin2hex(file_get_contents($hiki));
        $bins = str_split((string)$bin, 2);
        $i = 0;
        $ic = 0;
        $thistimestr = "";
        foreach($bins as $value)
        {
          ...
          if ($i == 1 || $i % 20 == 0)
          {
            ...
            if ($i != 1)
              echo " : ";
            $bin2 = @hex2bin($thistimestr);
            $bin2s = str_split($bin2, 2);
            foreach($bin2s as $value2)
    ...
    else if ($command == "remove-item" || $command == "removeitem")
    {
      ...
      if (File_Exists($hiki))
      {
        if (!unlink($hiki)) exit("Error: 指定されたファイルを削除できません。");
        else exit("");
      }
      ...
      for($i = 0; $i < count($_FILES["file"]["name"]); $i++ ){
        if(is_uploaded_file($_FILES["file"]["tmp_name"][$i])){
          ...
          if (move_uploaded_file($_FILES["file"]["tmp_name"][$i], $filename))
            ...
            echo $_FILES["file"]["name"][$i] . "をアップロードできませんでした。";
            $erro = true;
          }
        }
      }
    ...
    {
      session_write_close();
      if ($hiki == "remove-upload-info" && isset($_POST["FileName"]))
      {
        $jsonname = $_SESSION["cd"] . "/_upload-beacon_" . basename($_POST["FileName"]) . ".json.atkfmbeacon";
        if (is_file($jsonname) && unlink($jsonname))
          exit("");
        exit("error");
      }
      if(isset($_POST["filename"]) && is_uploaded_file($_FILES["data"]["tmp_name"])){
        ...
        fclose($a);
        $jsonname = $_SESSION["cd"] . "/_upload-beacon_" . basename($_POST["filename"]) . ".json.atkfmbeacon";
        @file_put_contents($jsonname, json_encode(array("size"=>filesize($filename))));
    ...
    else if ($command == "create-link" || $command == "createlink")
    {
      exit(file_put_contents($hiki, $_SESSION["cd"] . "/"));
    }
    ...
    else if ($command == "count-directory-files" || $command == "countdirectoryfiles")
    {
      ...
      $files = new RecursiveIteratorIterator(
            new RecursiveDirectoryIterator($hiki,
                FilesystemIterator::CURRENT_AS_FILEINFO |
                FilesystemIterator::KEY_AS_PATHNAME |
                FilesystemIterator::SKIP_DOTS
            ),
    ...
    else if ($command == "get-filemd5" || $command == "getfilemd5")
    {
      session_write_close();
      exit(md5_file($hiki));
    }
    else if ($command == "encrypt-item" || $command == "encryptitem")
    {
      session_write_close();
      file_put_contents($hiki . ".atkfm-encrypt", "ATKFMENCRYPTFILE!" . openssl_encrypt(file_get_contents($hiki), 'aes-256-cbc', $hiki2, OPENSSL_RAW_DATA, '4910857128499038'));
      exit($hiki . ".atkfm-encrypt");
    }
    else if ($command == "decrypt-item" || $command == "decryptitem")
    {
      session_write_close();
      $encrypt = pathinfo($hiki);
      if (!isset($encrypt["extension"]) || $encrypt["extension"] != "atkfm-encrypt")
        exit("エラー: 拡張子が「.atkfm-encrypt」のファイルを選択してください。現在の拡張子: " . $encrypt["extension"]);
      $fp = fopen($hiki, 'rb');
      if ($fp === false)
        exit("エラー: ファイルの読み込みに失敗しました。");
      fseek($fp, 0);
      $data = @fread($fp, 16);
      fclose($fp);
      if (filesize($hiki) < 19 || $data != "ATKFMENCRYPTFILE")
        exit("エラー: このファイルはATK-FMによって暗号化されたファイルではありません。");
      $dec = openssl_decrypt(substr(file_get_contents($hiki), 17), 'aes-256-cbc', $hiki2, OPENSSL_RAW_DATA, '4910857128499038');
      if ($dec == false)
        exit("エラー: パスワードが違う、又はファイルが破損しています。");
      file_put_contents($encrypt["dirname"] . "/" . $encrypt["filename"], $dec);
      exit("ファイルを複合化しました。");
    }
    ...
    else if ($command == "list-view" || $command == "listview")
    {
  ...
  <title><?=htmlspecialchars($hiki)?> | ファイルのプレビュー一覧 | ATK-FM</title>
  <pre><?=htmlspecialchars($hiki)?></pre>
  <?php
  $ir = 0;
  foreach($list as $value)
  {
    ...
    echo "統計ファイル番号: " . $ir . "\n";
    echo "ファイル名: " . $base . "\n";
    echo "サイズ: " . @calcFileSize(@filesize($value)) . " (" . @filesize($value) . "B)\n";
    echo "md5: " . @md5_file($value) . "\n";
    echo "</pre>";
    if ($ir > 500)
    {
      echo "(500件以降はサーバーに強い負荷がかかるため、非表示となっています。<a href='{$meurl}?ajax-typeof=download-item&ajax-option=" . urlencode($value) .  "' target='_blank'>ダウンロード</a>)";
      continue;
    }
    ...
    else if (filesize($value) > 10485760)
    {
      echo "(ファイルサイズが10MBを超えているため、非表示になっています。<a href='{$meurl}?ajax-typeof=get-item&ajax-option=" . urlencode($value) .  "' target='_blank'>プレビュー</a>)";
      continue;
    }
    $filepath['extension'] = strtolower($filepath['extension']);
    if ($filepath['extension'] == "png")
      echo "<img style='height:340px;' src='{$meurl}?ajax-typeof=get-item&ajax-option=" . urlencode($value) . "&accept=image/png'>";
    else if ($filepath['extension'] == "jpg" || $filepath['extension'] == "jpeg")
      echo "<img style='height:340px;' src='{$meurl}?ajax-typeof=get-item&ajax-option=" . urlencode($value) . "&accept=image/jpeg'>";
    else if (
      $filepath['extension'] == "txt" || $filepath['extension'] == "readme" || $filepath['extension'] == "md" ||
      $filepath['extension'] == "js" || $filepath['extension'] == "css" || $filepath['extension'] == "php" ||
      $filepath['extension'] == "json" || $filepath['extension'] == "c" || $filepath['extension'] == "cs" ||
      $filepath['extension'] == "py" || $filepath['extension'] == "go" || $filepath['extension'] == "vbs"
    )
    {
?>
<iframe style='width:604px;height:340px;' src="<?=$meurl?>?ajax-typeof=get-item-pre&ajax-option=<?=urlencode($value)?>"></iframe>
<?php
    }
    else if ($filepath['extension'] == "html" || $filepath['extension'] == "shtml" || $filepath['extension'] == "htm")
      echo "<iframe style='width:604px;height:340px;' sandbox src='{$meurl}?ajax-typeof=get-item&ajax-option=" . urlencode($value) . "&accept=text/html'></iframe>";
    else if ($filepath['extension'] == "pdf")
      echo "<iframe style='width:604px;height:340px;' src='{$meurl}?ajax-typeof=get-item&ajax-option=" . urlencode($value) . "&accept=application/pdf'></iframe>";
    else if ($filepath['extension'] == "gif")
      echo "<img style='height:340px;' src='{$meurl}?ajax-typeof=get-item&ajax-option=" . urlencode($value) . "&accept=image/gif'>";
      ...
      ?><audio controls src="<?=$meurl?>?ajax-typeof=get-item&ajax-option=<?=urlencode($value)?>&accept=audio/mp3">お使いのブラウザでは再生できません。
      <a href='<?=$meurl?>?ajax-typeof=download-item&ajax-option=<?=urlencode($value)?>' target='_blank'>ダウンロード</a></audio><?php
    }
  ...
  Debug_Write("[INFO] Loaded Ajax Tools");
...
<?php
  ...
  Debug_Write("[INFO] Loaded Forms Function");
  CheckUpdate(false, false);
  Debug_Write("[INFO] Loaded CheckUpdate(false, false)");
?>
<!DOCTYPE html>
<html lang="ja" dir="ltr">
  <head>
    ...
    var atkfm={};(function (window, undefined) {
      ...
      atkfm.MakeZip = function(ed) {
        ...
        .fail(
          function(e, n, t) {
            _("info").innerHTML="エラー:zip圧縮できませんでした。詳細 : " + t
          }
        )
      }

      atkfm.CloneItem = function() {
        _("info").innerHTML="URLからファイルをアップロードしています。。(この処理には数分間かかる場合があります)";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=uploadfromurl&ajax-option=" + encodeURIComponent(_("clone-url").value),
            type   : "get",
            success: function(e) {
              _("info").innerHTML=e;
              atkfm.GetFileList();
        ...
        .fail(
          function(e, n, t) {
            _("info").innerHTML="エラー:URLからファイルをアップロードできませんでした。詳細 : " + t;
          }
        );
      }
    
      atkfm.edit = function(e) {
        _("selecteditem").value = e;
        _("pp").style = "display:block;text-align:center;position:fixed;bottom:10%;left:10%;height:80%;width:80%;background-color:#00ff7f;color:#080808;overflow-x:hidden;overflow-y:visible;";
        _("run-file").style = "display:block";
      }

      atkfm.closep = function() {
        _("selecteditem").value = "";
        _("pp").style = "display:none;text-align:center;position:fixed;bottom:10%;left:10%;height:80%;width:80%;background-color:#00ff7f;color:#080808;overflow-x:hidden;overflow-y:visible;";
        _("run-file").style = "display:none;";
      }

      atk.vars_LinkTo = "";

      atkfm.GetFileList = function(pw) {
        let dds = "";
        if (pw)
          dds = "&ajax-option2=" + atk.encode(pw);
        if (_("directory-root").value.substr(-1,1) != "/")
          _("directory-root").value += "/";
        $.ajax(
          {
            url     : "<?=$meurl?>?ajax-typeof=get-directory&ajax-option=" + encodeURIComponent(_("directory-root").value) + dds,
            type    : "get",
            dataType: "json",
            success : function(e) {
              if (e["access"] == "ok") {
                _("info").innerHTML="アクセスに成功しました！";
                e["access"] = "s";
              }
              _("server-data").innerHTML = "";
              if (e["atk-fm-error"])
                _("info").innerHTML = e["atk-fm-error"];
              else if (e["access"] == "denied") {
                _("server-data").innerHTML = "";
                _("info").innerHTML = "フォルダーにアクセスするには" + e["admin"] + "さんの許可が必要です。" +
                  "[<a href='javascript:void(atkfm.NotDenied(\"" + _("directory-root").value + "\"))'>続行</a>]";
              }
              else if(e["access"] == "denied::PW_NOT_MATCH") {
                _("server-data").innerHTML = "";
                _("info").innerHTML = "パスワードが違います。フォルダーにアクセスするには" + e["admin"]+ "さんの許可が必要です。" +
                  "[<a href='javascript:void(atkfm.NotDenied(\""+_("directory-root").value+"\"))'>続行</a>]";
              }
              else
              {
                for(let n in e) {
                  let t = e[n];
                  o = n.slice(n.lastIndexOf("/")+1);
                  if ("a" == t)
                    _("server-data").innerHTML += "<img src='" + atkfm.images.fileimage +"' style='width:22px;height:22px;'> " + o +
                      "<a href='<?=$meurl?>?ajax-typeof=get-item&ajax-option=" + encodeURIComponent(n) + "' target='_blank' rel='noopener noreferrer'><input type='button' value='開く'></a>"+
                      "<a href='javascript:atkfm.edit(\""+n+"\");'><input type='button' value='操作'></a><br>";
                  else if ("b" == t)
                    _("server-data").innerHTML += "<img src='" + atkfm.images.ffimage + "' style='width:22px;height:22px;'>" + o +
                      '(フォルダー)<a href=\'javascript:_("directory-root").value="' + n + "\";atkfm.GetFileList();'><input type='button' value='開く'></a>" +
                      "<a href='javascript:atkfm.DirectoryRemove(\""+n+"\");'><input type='button' value='削除'></a>" +
                      "<a href='javascript:atkfm.MakeZip(\"" + n + "\");'><input type='button' value='圧縮'></a>" +
                      "<a href='javascript:atkfm.RenameItem(\"" + n + "\");'><input type='button' value='リネーム'></a>" +
                      "<a href='javascript:atkfm.CopyD(\"" + n + "\");'><input type='button' value='コピー'></a>" +
                      "<a href='javascript:atkfm.CountDirectoryFiles(\"" + n +"\");'><input type='button' value='詳細'></a><br>";
                  else if("c" == t)
                    ...
                    _("server-data").innerHTML += "<img src='" + atkfm.images.fzimagein + "' style='width:22px;height:22px;'> " + o +
                      ...
                      "&ajax-option2=" + encodeURIComponent(n) + "' target='_blank' rel='noopener noreferrer'><input type='button' value='表示'></a>" +
                      "<a href='javascript:atkfm.RemoveZip(\""+n+"\");'><input type='button' value='削除'></a><br>";
                  else if ("e" == t)
                    _("server-data").innerHTML += "<img src='" + atkfm.images.linkimage + "' style='width:22px;height:22px;'> " + o + "(リンク)<a href='javascript:" +
                      "atk.vars_LinkTo=atkfm.GetLinkTo(\"" + encodeURIComponent(n) +
                      "\");atk.vars_LinkTo.done(function(){if(atkfm.GetLink_Res!=\"\"){_(\"directory-root\").value=atk.decode(atkfm.GetLink_Res);" +
                      "atkfm.GetLink_Res=\"\";atkfm.GetFileList();}});" +
                      "'><input type='button' value='移動'></a><a href='javascript:" +
                      "atkfm.GetLinkTo(\"" + encodeURIComponent(n) +
                      "\").done(function(){_(\"info\").innerText=\"リンク先: \"+atk.decode(atkfm.GetLink_Res);" +
                      "atkfm.GetLink_Res=\"\";});" +
                      "'><input type='button' value='リンク先を表示'></a>" +
                      "<a href='javascript:atkfm.RenameItem(\""+n+"\");'><input type='button' value='リネーム'></a>" +
                      "<a href='javascript:atkfm.FileRemove(\""+n+"\");'><input type='button' value='削除'></a><br>";
                  else if ("f" == t)
                    _("server-data").innerHTML += "<img src='" + atkfm.images.encrypt + "' style='width:22px;height:22px;'> " + o +
                      "(暗号ファイル)<a href='javascript:atkfm.DecryptFile(\"" + encodeURIComponent(n) + "\");'><input type='button' value='復号化'></a>" +
                      "<a href='<?=$meurl?>?ajax-typeof=download-item&ajax-option="+encodeURIComponent(n)+"'><input type='button' value='ダウンロード'></a>" +
                      "<a href='javascript:atkfm.edit(\""+n+"\");'><input type='button' value='操作'></a><br>";
                  else
                    _("server-data").innerHTML += "(未対応のファイル)<br>"
                }
              }
            }
          }
        ...
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:データを更新できませんでした。詳細 : " + t;
          }
        )
      }
    
      atkfm.DirectoryRemove = function(e) {
        if (confirm("本当にディレクトリを削除しますか？"))
        {
          $.ajax(
            {
              url    : "<?=$meurl?>?ajax-typeof=remove-directory&ajax-option=" + encodeURIComponent(e),
              type   : "get",
              success: function(e) {
                _("info").innerHTML = "ディレクトリを削除しました。";
                atkfm.GetFileList()
              }
            }
          )
          .fail(
            function(e, n, t) {
              _("info").innerHTML = "エラー:ディレクトリを削除できませんでした。詳細 : " + t;
            }
          )
        }
      }

      atkfm.FileRemove = function(e) {
        if (confirm("本当にファイルを削除しますか？")) {
          $.ajax(
            {
              url    : "<?=$meurl?>?ajax-typeof=remove-item&ajax-option=" + encodeURIComponent(e),
              type   : "get",
              success: function(e) {
                _("info").innerHTML = e;
                atkfm.GetFileList();
          ...
          .fail(
            function(e, n, t) {
              _("info").innerHTML = "エラー:ファイルを削除できませんでした。詳細 : " + t
            }
          )
        }
      }

      atkfm.AddItem = function() {
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=add-item&ajax-option="+encodeURIComponent(_("add-item-name").value),
            type   : "get",
            success: function(e) {
              _("info").innerHTML = "ファイルを新規作成しました。";
              atkfm.GetFileList();
            }
          }
        )
        .fail(
          function(e, n, t) {
            _("info").innerHTML="エラー:ファイルを作成できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.MakeLink = function() {
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=create-link&ajax-option="+encodeURIComponent(_("linkto").value),
            type   : "get",
            success: function(e) {
              _("info").innerHTML = "リンクを作成しました。";
              atkfm.GetFileList();
            }
          }
        )
        .fail(
          function(e, n, t) {
            _("info").innerHTML="エラー:リンクを作成できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.UploadFile = function() {
        window.startuploaddata = new Date();
        _("info").innerHTML = "ファイルをアップロードしています。。(この処理には数分間かかる場合があります)";
        $.ajax(
          {
            xhr        : function() {
              ...
              xhr.upload.addEventListener("progress",
                function(evt) {
                  if (evt.lengthComputable) {
                    let byou = (new Date().getTime() - window.startuploaddata.getTime()) * 1000;
                    let speed = evt.loaded / byou;
                    parsent = evt.loaded / evt.total * 100;
                    _("config").innerText = "";
                    try{
                      _("config").innerText = "ConnectionType: " + navigator.connection.type +
                    ...
                    _("config").innerText +=
                      ...
                      "\nUpload %: " + parsent +
                      "\nUpload Speed: " + speed;
                    _("info").innerHTML = "ファイルをアップロードしています。。(" + parsent.toFixed(2) + "%完了、" + speed.toFixed(2) + "MB/s)";
                  }
                }, false
              );
              return xhr;
            },
            url        : "<?=$meurl?>?ajax-typeof=upload-item",
            type       : "POST",
            data       : new FormData($("#upload-file").get(0)),
            cache      : !1,
            contentType: !1,
            processData: !1,
            dataType   : "html"
        ...
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:ファイルをアップロードできませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.RenameItem = function(e, n) {
        if(!(n = prompt("パスを入力してください", e)))
          return false;
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=rename-item&ajax-option=" + encodeURIComponent(e) + "&ajax-option2=" + encodeURIComponent(n),
            type   : "get",
            success: function(e) {
              _("info").innerHTML = "ファイル名を変更しました。";
              atkfm.GetFileList();
            }
          }
        )
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:ファイル名を変更できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.CopyItem = function(e) {
        _("info").innerHTML = "ファイルをコピーしています。。";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=copy-item&ajax-option=" + encodeURIComponent(e),
            type   : "get",
            success: function(er) {
              _("info").innerHTML = "ファイルをコピーしました。";
              atkfm.GetFileList();
            }
          }
        )
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:ファイルをコピーできませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.EncryptFile = function(e) {
        var pw = window.prompt("【ファイル暗号化】\nファイルのパスワードを入力してください。");
        if (!pw)
        {
          _("info").innerHTML = "パスワードを空にする事はできません。";
          return false;
        }
        _("info").innerHTML = "ファイルの暗号化パスワードを計算しています。。";
        pw = CybozuLabs.MD5.calc(pw);
        pw = CybozuLabs.MD5.calc(pw);
        _("info").innerHTML = "ファイルを暗号化しています。。";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=encrypt-item&ajax-option=" + encodeURIComponent(e) + "&ajax-option2=" + encodeURIComponent(pw),
            type   : "get",
            success: function(er) {
              _("info").innerHTML = "ファイルを暗号化しました。";
              atkfm.GetFileList();
            }
          }
        )
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:ファイルを暗号化できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.DecryptFile = function(e) {
        var pw = window.prompt("【ファイル複合化】\nファイルのパスワードを入力してください。");
        if (!pw)
        {
          _("info").innerHTML = "パスワードを空にする事はできません。";
          return false;
        }
        _("info").innerHTML = "ファイルの複合化パスワードを計算しています。。";
        pw = CybozuLabs.MD5.calc(pw);
        pw = CybozuLabs.MD5.calc(pw);
        _("info").innerHTML = "ファイルを複合化しています。。";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=decrypt-item&ajax-option=" + encodeURIComponent(e) + "&ajax-option2=" + encodeURIComponent(pw),
            type   : "get",
            success: function(er) {
              _("info").innerText = er;
              atkfm.GetFileList();
        ...
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:ファイルを複合化できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.AddDirectory = function(e) {
        if ("/" != _("directory-root").value.substr(-1,1))
          _("add-directory-name").value += "/";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=add-directory&ajax-option=" + encodeURIComponent(e),
            type   : "get",
            success: function(er) {
              _("info").innerHTML = "ディレクトリを新規作成しました。";
              atkfm.GetFileList();
            }
          }
        )
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:ディレクトリを作成できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.CountDirectoryFiles = function(e) {
        _("info").innerHTML = "詳細情報を取得しています。。";
        $.ajax(
          {
            url     : "<?=$meurl?>?ajax-typeof=count-directory-files&ajax-option=" + encodeURIComponent(e),
            type    : "get",
            dataType: "json",
            success : function(e) {
              _("info").innerText = e.File + "個のファイル、" + e.Directory + "個のディレクトリがあります。計測時間:" + e.Time.toFixed(3) + "秒、サイズ:" + e.SIZE
            }
          }
        )
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:詳細情報を取得できませんでした。詳細 : " + t;
          }
        )
      }
    
      atkfm.GetSize = function(e) {
        _("info").innerHTML = "ファイルのサイズを取得しています。。";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=get-filesize&ajax-option=" + encodeURIComponent(e),
            type   : "get",
            success: function(e) {
              _("info").innerText = "ファイルのサイズ: " + e;
            }
          }
        )
        .fail(
          function(e, n, t){
            _("info").innerHTML = "エラー:ファイルのサイズを取得できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.GetMD5 = function(e) {
        _("info").innerHTML = "ファイルのmd5ハッシュを取得しています。。";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=get-filemd5&ajax-option=" + encodeURIComponent(e),
            type   : "get",
            success: function(e) {
              _("info").innerText = "ファイルのmd5ハッシュ: " + e;
            }
          }
        )
        .fail(
          function(e, n, t){
            _("info").innerHTML = "エラー:ファイルのmd5ハッシュを取得できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.OpenZip = function(e) {
        _("info").innerHTML = "展開しています。。";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=open-zip&ajax-option=" + encodeURIComponent(e),
            type   : "get",
            success: function(e) {
              _("info").innerText = e;
              atkfm.GetFileList();
        ...
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:ファイルを展開できませんでした。詳細 : " + t;
          }
        )
      }

      atkfm.RemoveZip = function(e) {
        if (confirm("本当にファイルを削除しますか？"))
          _("info").innerHTML = "削除しています。。";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=remove-item-zip&ajax-option=" + encodeURIComponent(_("directory-root").value) + "&ajax-option2=" + encodeURIComponent(e), 
            type   : "get",
            success: function(e) {
              _("info").innerText = e;
              atkfm.GetFileList();
      ...
      atkfm.CopyD = function(e) {
        _("info").innerHTML = "ディレクトリをコピーしています。。";
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=copy-item&ajax-option=" + encodeURIComponent(e),
            type   : "get",
            success: function(e) {
              _("info").innerHTML = "ディレクトリをコピーしました。";
              atkfm.GetFileList();
            }
          }
        )
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:ディレクトリをコピーできませんでした。詳細 : " + t;
          }
        )
      }
      ...
      atkfm.GetLinkTo = function(e) {
        _("info").innerHTML = "リンク先を取得しています。。";
        let def = new $.Deferred();
        $.ajax(
          {
            url    : "<?=$meurl?>?ajax-typeof=get-linkto&ajax-option=" + encodeURIComponent(e),
            type   : "get",
            success: function(e) {
              atkfm.GetLink_Res = e;
            }
        ...
        .fail(
          function(e, n, t) {
            _("info").innerHTML = "エラー:リンク先を取得できませんでした。詳細 : " + t;
            atkfm.GetLink_Res = "";
          }
        )
        .always(
      ...
      atkfm.NotDenied = function(e) {
        let s = prompt("続行するにはパスワードを入力してください");
        if(s == null || s == false)
          return false;
        atkfm.GetFileList(s);
      }

      atkfm.AccessCt = function() {
        _("accessdenysetting").style = "display:block;text-align:center;position:fixed;bottom:10%;left:10%;height:80%;width:80%;background-color:#00ff7f;color:#080808;overflow-x:hidden;overflow-y:visible;";
      }

      atkfm.closep2 = function() {
        _("accessdenysetting").style = "display:none;text-align:center;position:fixed;bottom:10%;left:10%;height:80%;width:80%;background-color:#00ff7f;color:#080808;overflow-x:hidden;overflow-y:visible;";
      }

      atkfm.SendBeacon_UploadFinished = function(filename) {
        let sr = "<?=$meurl?>?ajax-typeof=min-upload-item&ajax-option=remove-upload-info";
        let bcend = new FormData();
        bcend.append('FileName', filename);
        try {
          navigator.sendBeacon(sr, bcend);
        } catch(_e) {
          try{
            navigator.sendBeacon(sr, bcend);
          } catch(__e) {
      ...
      window.onload = function() {
        ...
        (
          ()=>{
            let e = 0, t = 0;
            _("min-upload-file").onsubmit = (
              async i => {
                i.preventDefault();
                let a = _("file2").files[0];
                e = a.size;
                let n = 1 * _("upload-mb").value * 1024 * 1024;
                t = Math.ceil(e / n);
                for(let e = 0;e < t; e++) {
                  let i = new FormData;
                  i.append("data", a.slice(e * n,(e + 1) * n)),
                  i.append("filename", _("file2").files[0].name),
                  _("info").innerText = "ファイルを分割アップロードしています。。(" + Math.ceil(100 * (e + 1) / t).toString() + "%完了)",
                  _("config").innerText = "Upload %: " + Math.ceil(100 * (e + 1) / t).toString() + "\nUploaded/Total: " + e + "/" + t;
                  await fetch(
                    "<?=$meurl?>?ajax-typeof=min-upload-item",
                    {
                      body: i,
                      method: "POST",
                      headers: {
                  ...
                  .catch(
                    error1 => {
                      console.error("ATKFM.Exception: Upload.Try1: " + error1);
                      _("config").innerText += "\n* Warn: ATKFM.Exception: Upload.Try1: " + error1;
                      fetch(
                        "<?=$meurl?>?ajax-typeof=min-upload-item",
                        {
                          body: i,
                      ...
                      .catch(
                        error2 => {
                          console.error("ATKFM.Exception: Upload.Try2: " + error2);
                          _("config").innerText += "\n* Warn: ATKFM.Exception: Upload.Try2: " + error2;
                          fetch(
                            "<?=$meurl?>?ajax-typeof=min-upload-item",
                            {
                              body: i,
                          ...
                          .catch(
                            error3 => {
                              console.error("ATKFM.Exception: Upload.Try3: " + error3);
                              _("config").innerText += "\n* !Fatal: ATKFM.Exception: Upload.Try3: " + error3;
                              _("info").innerText = "エラー: 分割アップロードできませんでした。ネットワークを確認してください。";
                              return false;
                            }
                          );
                        }
                ...
                return atkfm.SendBeacon_UploadFinished(_("file2").files[0].name),
                _("info").innerText="ファイルを分割アップロードしました！",
                _("file2").value="",
                atkfm.GetFileList(),
                !1
              }
  ...
  <body style="background-color:#e6e6fa;text:#363636;">
    <form action='<?=$meurl?>' method='POST'>
      <span><?=$title?></span>
      <input type="button" value="更新" onclick="atkfm.GetFileList();">
    ...
    <div id="client" style="text-align:left;position:fixed;bottom:2%;left:02%;height:92%;width:46%;background-color:#6495ed;color:#080808;overflow-x:hidden;overflow-y:visible;">
      <form action='<?=$meurl?>' method='POST' onsubmit="atkfm.AddItem();return false;">
        ファイル名: <input type="text" name="add-item-name" id="add-item-name" value="example.txt" size="40">
        <input type='button' onclick="atkfm.AddItem();" value="新規作成">
      </form>
      <form action='<?=$meurl?>' method='POST' onsubmit="atkfm.AddDirectory(_('add-directory-name').value);return false;">
        ディレクトリ名: <input type="text" name="add-directory-name" id="add-directory-name" value="example/" size="40">
        <input type='button' onclick="atkfm.AddDirectory(_('add-directory-name').value);" value="新規作成">
      </form>
      <form action="<?=$meurl?>" id="upload-file" name="upload-file" enctype="multipart/form-data" method="POST" onsubmit="atkfm.UploadFile();return false;">
        ...
        <input type="button" onclick="atkfm.UploadFile();" value="保存">
      </form>
      ...
      <form action='<?=$meurl?>' method='POST' onsubmit="atkfm.CloneItem();return false;">
        URLからアップロード: <input type="text" name="clone-url" id="clone-url" placeholder="URLを入力してください" size="40">
        <input type='button' onclick="atkfm.CloneItem();" value="保存">
      </form>
      <form action='<?=$meurl?>' method='POST' onsubmit="atkfm.MakeLink();return false;">
        このフォルダーへのショートカットを作成: <br><input type="text" name="linkto" id="linkto" value="<?=$_SESSION["cd"]?>/example.atkfm-link" placeholder="リンクファイルのパスを入力してください" style="width:80%;">
        <input type='button' onclick="atkfm.MakeLink();" value="作成">
      </form>
      アクセス制限: <a href='javascript:return false;void(atkfm.AccessCt())' title="アクセス制限が簡単にすり抜けられる方法が見つかったので、未実装です">
    ...
    <div id="server" style="text-align:left;position:fixed;bottom:2%;left:50%;height:92%;width:46%;background-color:#6495ed;color:#080808;overflow-x:hidden;overflow-y:visible;">
      <form action='<?=$meurl?>' method='POST' onsubmit='atkfm.GetFileList();return false;'>
        <input type="text" name="ajax-option" id="directory-root" value="<?=$_SESSION["cd"]?>" size="50">
        <input type='button' onclick="atkfm.GetFileList();" value="送信">
        <input type='button' onclick='"/"==_("directory-root").value.substr(-1,1)&&(_("directory-root").value=_("directory-root").value.slice(0,-1)),_("directory-root").value=_("directory-root").value.substring(0,_("directory-root").value.lastIndexOf("/")),atkfm.GetFileList();' value="上へ">
        <input type='button' onclick='atkfm.CountDirectoryFiles(_("directory-root").value);' value="詳細">
        <input type='button' onclick='window.open("<?$meurl?>?ajax-typeof=list-view&ajax-option=" + atk.encode(_("directory-root").value), "_blank");' value="閲覧">
      </form>
    ...
    <div id="pp" style="display:none;text-align:center;position:fixed;bottom:10%;left:10%;height:80%;width:80%;background-color:#00ff7f;color:#080808;overflow-x:hidden;overflow-y:visible;">
      <div align="right" style="text-align:right;">
        <input type='button' onclick="atkfm.closep();" value="閉じる" style="height:50px;width:80px;">
      </div>
      <h1>操作</h1>
      <div id="options">
        ...
        <div id="run-file" style="display:none;">
          <a href='javascript:window.open("<?=$meurl?>?ajax-typeof=get-item&ajax-option="+encodeURIComponent(_("selecteditem").value), "_blank");atkfm.closep();'>
            <input type='button' value='開く(編集モード)'>
          </a>
          <a href='javascript:window.open("<?=$meurl?>?ajax-typeof=get-item&readonly=true&ajax-option="+encodeURIComponent(_("selecteditem").value), "_blank");atkfm.closep();'>
            <input type='button' value='読み取り専用で開く'>
          </a>
          <br>
          ファイルをエディタで開きます。読み取り専用で開くと編集できなくなります。
          <br>
          <a href='javascript:window.open("<?=$meurl?>?ajax-typeof=view-html&ajax-option="+encodeURIComponent(_("selecteditem").value), "_blank");atkfm.closep();'>
            <input type='button' value='HTML表示'>
          </a>
          <br>
          HTML形式で直接レスポンスします。PHP等は動作しません。
          <br>
          <a href='javascript:window.open("<?=$meurl?>?ajax-typeof=view-hex&ajax-option="+encodeURIComponent(_("selecteditem").value), "_blank");atkfm.closep();'>
            <input type='button' value='バイナリ表示'>
          </a>
          <br>
          バイナリで表示します。読み取り専用です。
          <br>
          <a href='javascript:atkfm.FileRemove(_("selecteditem").value);atkfm.closep();'>
            <input type='button' value='削除'>
          </a>
          <br>
          ファイルを削除します。
          <br>
          <a href='javascript:window.open("<?=$meurl?>?ajax-typeof=download-item&ajax-option="+encodeURIComponent(_("selecteditem").value));atkfm.closep();'>
            <input type='button' value='ダウンロード'>
          </a>
          <br>
          ファイルをダウンロードします。ブラウザによっては正常にダウンロードされず、プレビューされてしまう場合があります。
          <br>
          <a href='javascript:atkfm.RenameItem(_("selecteditem").value);atkfm.closep();'>
            <input type='button' value='リネーム'>
          </a>
          <br>
          ファイル名を変更できます。ディレクトリから指定するので、移動させる事も可能です。
          <br>
          <a href='javascript:atkfm.CopyItem(_("selecteditem").value);atkfm.closep();'>
            <input type='button' value='コピー'>
          </a>
          <br>
          ファイルをコピーできます。
          <br>
          <a href='javascript:atkfm.GetSize(_("selecteditem").value);atkfm.closep();'>
            <input type='button' value='ファイルサイズ取得'>
          </a>
          <br>
          ファイルサイズを取得/表示します。
          <br>
          <a href='javascript:atkfm.GetMD5(_("selecteditem").value);atkfm.closep();'>
            <input type='button' value='md5ハッシュ表示'>
          </a>
          <br>
          md5ハッシュを取得/表示します。
          <br>
          <a href='javascript:atkfm.EncryptFile(_("selecteditem").value);atkfm.closep();'>
            <input type='button' value='暗号化'>
          </a>
          <br>
          パスワードによってファイルをAES(鍵長256bit、Cipher-Feedbackモード)で複雑に暗号化します。<br>
          ※注意！パスワードを忘れた場合、ファイルは二度と復元できなくなる可能性があります。
        </div>
      </div>
    </div>
...
<html>
  <head>
    ...
    <script type="text/javascript">var starttitle="<?=$title?>";function save(){_("info").innerHTML="保存しています。。",$.ajax({url:"<?=$meurl?>?ajax-typeof=save-item&ajax-option="+encodeURIComponent(_("pt").value),type:"post",data:{naka:_("naka").value},success:function(t){""==t?(_("info").innerHTML="変更を保存しました。",$("title").html(starttitle)):_("info").innerHTML="変更を保存できませんでした。"}}).fail(function(t,n,e){_("info").innerHTML="エラー:変更を保存できませんでした。詳細: "+e,alert("変更を保存できませんでした。")})}$(document).ready(function(){$("#naka").on("change",function(){$("title").html("*"+starttitle)})});document.onkeydown=function(e){if(event.ctrlKey&&83==event.keyCode)return save(),event.keyCode=0,!1};function newfm(){window.open("<?=$meurl?>?ajax-typeof=get-item&ajax-option="+atk.encode(_("pt").value),"_blank")}</script>
    <?php } ?>
  ...
  <body>
    <form action="<?=$meurl?>?ajax-type=get-item&ajax-option=<?php echo $filepath; ?>" method="POST" onsubmit="save();return false;">
      ...
      <div style="position:fixed;bottom:3%;left:40px;">
        ...
        <input type="button" value="置き換え" <?php if (isset($_GET["readonly"])) echo "disabled"; ?> onclick='let e=_("naka").value,n=window.prompt("置き換えるテキストを入力してください"),o=window.prompt("置き換え後のテキストを入力してください");if(n){for(p=e.replace(n,o);p!==e;)e=e.replace(n,o),p=p.replace(n,o);_("naka").value=p}'>
  ...
  <body style="background-color:#e6e6fa;text:#363636;">
    ...
    <?php
      try
      {
        ...
        無料のレンタルサーバーなどではPHP.iniによって実行できなくされている可能性があります。
      </div><?php
      } catch (\Throwable $e) {
      ?>
      <div align="center">
